...
# MAGIC To run this demo, just select the cluster `dbdemos-lakehouse-iot-platform-andrew_hahn` from the dropdown menu ([open cluster configuration](https://e2-demo-field-eng.cloud.databricks.com/#setting/clusters/0412-152949-6pf51tmu/configuration)). <br />
...
# MAGIC
# MAGIC Let's review our dataset and start analyze the data we have to predict our equipment failures
...
def plot(sensor_report):
  ...
  df = spark.table('historical_sensor_bronze').where(f"turbine_id == '{turbine_id}' ").orderBy('timestamp').pandas_api()
...
# Read our training features table
...
g = sns.PairGrid(turbine_dataset.sample(0.05).toPandas()[['std_sensor_A', 'std_sensor_E', 'damaged','avg_energy']], diag_sharey=False, hue="damaged")
