...
-- MAGIC %md
-- MAGIC Your DLT Pipeline has been installed and started for you! Open the <a dbdemos-pipeline-id="dlt-iot-wind-turbine" href="#joblist/pipelines/3d2f7f37-addd-4f78-aa7a-3c66151bd10c" target="_blank">IOT Wind Turbine Delta Live Table pipeline</a> to see it in action.<br/>
-- MAGIC
...
CREATE STREAMING TABLE parts 
COMMENT "LM2500 GTE parts from our manufacturing system"
AS SELECT * FROM stream(shared_navy_demo.dbdemos_navy_pdm.parts)
...
-- MAGIC #                                                                      Model name                                         |        
-- MAGIC #                                                                                         |                               |        
-- MAGIC predict_maintenance_udf = mlflow.pyfunc.spark_udf(spark, "models:/ahahn_demo.dbdemos_navy_pdm.navy_turbine_maintenance@prod", "string") #, env_manager='virtualenv'
...
SELECT * EXCEPT(m.row_number), 
    predict_maintenance(turbine_id, hourly_timestamp, avg_energy, std_sensor_A, std_sensor_B, std_sensor_C, std_sensor_D, std_sensor_E, std_sensor_F, percentiles_sensor_A, percentiles_sensor_B, percentiles_sensor_C, percentiles_sensor_D, percentiles_sensor_E, percentiles_sensor_F) as prediction 
...
-- MAGIC
-- MAGIC ## Conclusion
-- MAGIC Our <a dbdemos-pipeline-id="dlt-iot-wind-turbine" href="#joblist/pipelines/3d2f7f37-addd-4f78-aa7a-3c66151bd10c" target="_blank">DLT Data Pipeline</a> is now ready using purely SQL. We have an end 2 end cycle, and our ML model has been integrated seamlessly by our Data Engineering team.
-- MAGIC
-- MAGIC
...
-- MAGIC
-- MAGIC We are now ready to build our <a href="/sql/dashboards/62aa0c60-35a8-4436-80a2-4ec91f3e8068">DBSQL Dashboard</a>to track the main KPIs and status of our entire Wind Turbine Farm and build complete <a href="/sql/dashboards/9dfc30b5-9259-49ab-bf1c-b3508203f51f">Predictive maintenance DBSQL Dashboard</a>.
-- MAGIC
-- MAGIC
