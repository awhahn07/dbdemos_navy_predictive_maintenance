...
-- MAGIC
-- MAGIC The Dashboard has been loaded for you. Open the [Turbine DBSQL Dashboard](/sql/dashboards/62aa0c60-35a8-4436-80a2-4ec91f3e8068) to start reviewing our our Wind Turbine Farm stats or the [DBSQL Predictive maintenance Dashboard](/sql/dashboards/9dfc30b5-9259-49ab-bf1c-b3508203f51f).
...
-- MAGIC %md
-- MAGIC
-- MAGIC # Taking our analysis one step further: Predicting Maintenance Activities
-- MAGIC
-- MAGIC Being able to run analysis on our past data already gives us a lot of insight. We can better understand the sensor data that coorelated with equipment failure for overall Force readiness impact.
-- MAGIC
-- MAGIC However, knowing which equipment needs maintenance isn't enough. We now need to take it to the next level and build a predictive model to determine which equipment is predicted to fail and how to take prescriptive action for maintenance activity.
-- MAGIC
