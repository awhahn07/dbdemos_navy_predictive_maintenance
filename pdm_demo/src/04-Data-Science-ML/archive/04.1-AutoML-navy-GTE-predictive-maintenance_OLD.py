...
# MAGIC <img width="1px" src="https://ppxrzfxige.execute-api.us-west-2.amazonaws.com/v1/analytics?category=lakehouse&org_id=local&notebook=%2F04-Data-Science-ML%2F04.1-automl-iot-turbine-predictive-maintenance&demo_name=lakehouse-iot-platform&event=VIEW&path=%2F_dbdemos%2Flakehouse%2Flakehouse-iot-platform%2F04-Data-Science-ML%2F04.1-automl-iot-turbine-predictive-maintenance&version=1">

# COMMAND ----------

# MAGIC %run ../_resources/00-load-tables $reset_all_data=false $catalog="hive_metastore" $db="dbdemos_navy_turbine_andrew_hahn"
...
dataset = dataset.drop(columns=['end_time', 'start_time', '_rescued_data', 'country', 'lat', 'long', 'damaged', 'location', 'model', 'state'])
# Drop missing values
...
# MAGIC
# MAGIC ### Using Databricks Auto ML with our GTE dataset
# MAGIC
...
display_automl_turbine_maintenance_link(dataset = fs.read_table(f'{database}.turbine_hourly_features'), model_name = "dbdemos_navy_turbine_maintenance", force_refresh = True)

# COMMAND ----------

# MAGIC %md
# MAGIC AutoML saved our best model in the MLFlow registry. [Open the dbdemos_turbine_maintenance model](#mlflow/models/dbdemos_turbine_maintenance) to explore its artifact and analyze the parameters used, including traceability to the notebook used for its creation. [Explore the experiment used to generate the model](#https://e2-demo-field-eng.cloud.databricks.com/ml/experiments/2196073487824492?o=1444828305810485&searchFilter=&orderByKey=metrics.%60val_f1_score%60&orderByAsc=false&startTime=ALL&lifecycleFilter=Active&modelVersionFilter=All+Runs&datasetsFilter=W10%3D)
# MAGIC
